...
inline void stm_tune_scheduler(){
	...
	if(th_minus_2 >= th && th_minus_2 >= th_minus_1) {
		...
		else u_m = ((float)total_tx_time/(float)1000000)/(float)commited_txs;
	...
	*/
	tx->start_no_tx_time=STM_TIMER_READ();
	printf("\nCommitted %i, ticks %i, time %f",commited_txs, now-last_tuning_time, (float)(now-last_tuning_time)/(float)1000000), tx_info_table[0][1]);
