...
inline void stm_wait(int id) {
	...
	if(entered==0){
		...
		if (//(tx->i_am_the_collector_thread!=1) &&
				...
				queued_transactions>busy_waiting_time_threashold) {
				//sleeping
...
inline void stm_tune_scheduler(){
	...
	printf("\nTx time: %f, Spin time: %f, No tx time %f", ((float)total_tx_time+(float)total_tx_wasted_time)/(float)total_committed_transactions, (float)total_tx_spin_time/(float)total_committed_transactions, (float)total_no_tx_time/(float)total_committed_transactions,
			((float)total_tx_spin_time/(float)total_committed_transactions)/((float)total_tx_spin_time/(float)total_committed_transactions+(float)total_no_tx_time/(float)total_committed_transactions));
	/*
