...
inline void stm_wait(int id) {
	...
	if(entered==0){
		...
		printf("\nbusy_waiting_transactions %i, max_allowed_running_transactions %i", busy_waiting_transactions, max_allowed_running_transactions);
		fflush(stdout);

		if (//(tx->i_am_the_collector_thread!=1) &&
				(0 && (float)(busy_waiting_transactions)>(float)busy_waiting_time_threashold)) {
			...
			printf("\nbusy_waiting_transactions-1: %i, Average spin time per waiting transaction %f, product %f, thread slept for ticks=%llu",
					busy_waiting_transactions-1,
					(double)average_spin_time_per_waiting_transacton,
					(double)(busy_waiting_transactions-1) * (double)average_spin_time_per_waiting_transacton,
					end-start);
			fflush(stdout);
			*/
